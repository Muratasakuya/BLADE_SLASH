...
void BossEnemyAttackSign::Init() {
	...
	for (auto& sign : signs_) {
		...
		sign = std::make_unique<SakuEngine::GameObject2D>();
		...
		// 色
		sign->SetColor(SakuEngine::Color::Convert(0xF06C0AFF));
		sign->SetEmissionColor(SakuEngine::Vector3(sign->GetColor().r,
...
void BossEnemyAttackSign::Emit(const SakuEngine::Vector2& emitPos) {
...
void BossEnemyAttackSign::UpdateAnimation() {
	...
	const SakuEngine::Vector2 baseSize = SakuEngine::Vector2::Lerp(size_.start, size_.target, t);
	...
	for (uint32_t i = 0; i < static_cast<uint32_t>(SignDirection::Count); ++i) {
		...
		// サイズ、横長か縦長で処理を分ける
		const SakuEngine::Vector2 size = info.horizontal ? SakuEngine::Vector2(baseSize.y, baseSize.x)
			: SakuEngine::Vector2(baseSize.x, baseSize.y);

		// 進行方向へ移動
		const SakuEngine::Vector2 pos = emitPos_ + info.direction * move;
...
BossEnemyAttackSign::DirectionInfo BossEnemyAttackSign::GetDirectionInfo(SignDirection direction) const {
	...
	switch (direction) {
	case SignDirection::Up:     return { SakuEngine::Vector2(0.0f, -1.0f), SakuEngine::Vector2::AnyInit(0.5f),  false };
	case SignDirection::Bottom: return { SakuEngine::Vector2(0.0f,  1.0f), SakuEngine::Vector2::AnyInit(0.5f),  false };
	case SignDirection::Right:  return { SakuEngine::Vector2(1.0f, 0.0f), SakuEngine::Vector2::AnyInit(0.5f),  true };
	case SignDirection::Left:   return { SakuEngine::Vector2(-1.0f, 0.0f), SakuEngine::Vector2::AnyInit(0.5f),  true };
	default:                    return { SakuEngine::Vector2::AnyInit(0.0f), SakuEngine::Vector2::AnyInit(0.5f),  false };
	}
}
...
void BossEnemyAttackSign::ImGui() {
	...
	if (ImGui::Button("Emit")) {
		...
		Emit(SakuEngine::Vector2(Config::kWindowWidthf / 2.0f, Config::kWindowHeightf / 2.0f));
	}

	ImGui::Text("currnetState: %s", SakuEngine::EnumAdapter<State>::ToString(currentState_));
...
void BossEnemyAttackSign::ApplyJson() {
	...
	Json data;
	if (!SakuEngine::JsonAdapter::LoadCheck("Enemy/Boss/bossEnemyAttackSign.json", data)) {
		return;
	}

	animationTimer_.FromJson(data["AnimationTimer"]);
	size_.start = SakuEngine::Vector2::FromJson(data["size_start"]);
	size_.target = SakuEngine::Vector2::FromJson(data["size_target"]);
...
void BossEnemyAttackSign::SaveJson() {
	...
	SakuEngine::JsonAdapter::Save("Enemy/Boss/bossEnemyAttackSign.json", data);
}